...
c.reset(); c.resize(800,3200); close(c.current[2],1); c.resize(800,1600); close(c.current[2],1); // layout preserves camera
...
const focus=new Controls();focus.current=[.7,.2,1,.1,.2,.3];focus.target=focus.current.slice();
function focusEye(c){const [y,p,z,x,Y,Z]=c.current;return [x+Math.sin(y)*Math.cos(p)*3*z,Y+Math.sin(p)*3*z,Z+Math.cos(y)*Math.cos(p)*3*z];}
const e=focusEye(focus);focus.focus([.3,.2,.1]);const e2=focusEye(focus);e.forEach((v,i)=>close(v,e2[i]));
