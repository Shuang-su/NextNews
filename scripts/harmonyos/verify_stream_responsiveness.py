...
for line in log.splitlines():
 ...
 m=re.search(r'StreamFrame state=(\w+) frames=(\d+) count=(\d+) loadMs=([\d.]+) frameMs=([\d.]+)(?: uploadMs=([\d.]+))?(?: uploadedRows=(\d+) reusedRows=(\d+) decodedFiles=(\d+) subsetHits=(\d+))?',line)
 if m:rows.append([parts[1],m[1],int(m[2]),int(m[3]),float(m[4]),float(m[5]),float(m[6]) if m[6] else None]+[int(m[i]) if m[i] else None for i in range(7,11)])
with (a.out/'frames.csv').open('w') as f:
 writer=csv.writer(f,lineterminator="\n");writer.writerow(['time','state','frames','resident_count','last_load_ms','last_draw_ms','last_upload_batch_ms','uploaded_rows','reused_rows','decoded_files','subset_hits']);writer.writerows(rows)
