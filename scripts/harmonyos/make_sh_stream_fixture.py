...
p=argparse.ArgumentParser();p.add_argument('directory',type=Path);p.add_argument('--highest-band',action='store_true');args=p.parse_args();args.directory.mkdir(parents=True,exist_ok=False)
chunks=[]
for degree in [1,2,3]:
 path=args.directory/f'chunk-{degree:04d}.sog'
 subprocess.run([sys.executable,str(Path(__file__).with_name('make_sog_sh_fixture.py')),str(path),'--degree',str(degree)]+(['--highest-band'] if args.highest_band else []),check=True)
...
(args.directory/'provenance.json').write_text(json.dumps(dict(source='NextNews original synthetic planes',purpose='Independent source SH1/2/3 books; nonzero highest-band zonal terms' if args.highest_band else 'Independent source SH1/2/3 books and stable-page residency; nonzero first-order terms only'),indent=2))
