...
icon('模型');click('公开样例');time.sleep(1);icon('模型');snap('public-opengl')
icon('模型');click('切换华为原生');time.sleep(2);icon('模型');snap('public-huawei')
icon('模型');click('切换 OpenGL');time.sleep(2);click('转换样例');time.sleep(1);click('配置默认镜头');icon('模型');before=snap('annotations-overview')
...
shell('uitest uiInput keyEvent Home');time.sleep(1);shell('aa start -b com.nextnews.splatviewer -a EntryAbility');time.sleep(2);snap('huawei-resumed')
icon('模型');click('切换 OpenGL');time.sleep(2);icon('模型');icon('飞行');icon('设置');click('游戏控制：关');icon('设置');snap('gaming-controls')
...
(a.out/'result.json').write_text(json.dumps({'uiSmoke':'passed','annotations':10,'backends':['OpenGL','Huawei'],'visualReviewRequired':True,'performanceGatesTested':False},indent=2))
