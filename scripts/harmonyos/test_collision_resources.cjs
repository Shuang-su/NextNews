...
const {validateCollisionManifest:validate,collisionWorldBox}=load('ViewerCollision',{'./ViewerRequests':{viewerRequests:pool}});
...
const bounds={min:[1,2,3],max:[4,5,6]},original=JSON.stringify(bounds);
assert.equal(JSON.stringify(collisionWorldBox(bounds,'metaflow-rz180')),JSON.stringify({min:[-4,-5,3],max:[-1,-2,6]}));
assert.equal(JSON.stringify(collisionWorldBox(bounds,'world')),original);assert.equal(JSON.stringify(bounds),original);
console.log('PASS explicit collision AABB transformation without mutating original metadata');
