...
int main(int argc,char **argv){
    if(argc==2){std::ifstream file(argv[1]);nlohmann::json m;file>>m;LodTree tree;tree.bounds=m["bounds"].get<std::array<double,4>>();tree.levels=m["levels"];
        ...
        nlohmann::json result=nlohmann::json::array();for(const auto yaw:{0.,.846,-.65,3.14159265358979323846})for(const auto budget:{2000000u,4000000u})result.push_back(tree.Select({yaw,.056,1,.412,-.139,.446,1},budget,75,.5032405642394204));std::cout<<result.dump();return 0;
    }
