...
void Renderer::AdvancePages(){
    ...
    if(!work.planned){
        ...
        if(work.encoded){
            ...
            texture(encodedCenters_,GL_RGBA32F,4096,encodedRows_);texture(encodedCodes_,GL_RGBA32UI,4096,encodedRows_);texture(codebookTexture_,GL_RG32F,256,2048);
    ...
    auto bookReady=[&](uint32_t index,size_t reserved){
        ...
        if(uploaded+reserved+2048>budget)return false;
        const auto &table=*work.pages[index]->tables;std::array<float,512> values{};
        for(int i=0;i<256;++i){values[i*2]=std::exp(2*table.scale[i]);values[i*2+1]=std::clamp(.5f+.28209479177387814f*table.color[i],0.f,1.f);}
        glBindTexture(GL_TEXTURE_2D,codebookTexture_);glTexSubImage2D(GL_TEXTURE_2D,0,0,work.bookSlots[index],256,1,GL_RG,GL_FLOAT,values.data());
        if(glGetError()!=GL_NO_ERROR)throw std::runtime_error("SOG codebook upload failed");
        bookAtlas_.MarkReady(key);uploaded+=2048;work.uploaded+=2048;return true;
    };
    ...
    while(work.cursor<work.keys.size()){
        const uint32_t index=work.uploadOrder[work.cursor];
        if(!bookReady(index,0))break;
        ...
        while(work.cursor<work.keys.size()&&slot%pagesPerRow+run<pagesPerRow){
            ...
            if(nextSlot!=slot+run||pageReady(n)||uploaded+(run+1)*pageBytes>budget||!bookReady(n,(run+1)*pageBytes))break;
            ...
            for(size_t i=0;i<page.Count();++i){const auto point=run*PagePoints+i;
                if(work.encoded){std::copy_n(page.Position(i),3,pixels.data()+point*4);const auto &c=page.codes[i];
                    codes[point*4]=c.quaternion;codes[point*4+1]=c.scale;codes[point*4+2]=c.color;codes[point*4+3]=work.bookSlots[n];}
    ...
    if(work.cursor==work.keys.size()&&work.generation==loadGeneration_){
        ...
        retiredScenes_.push_back(std::move(scene_));scene_=work.scene;if(scene_->Count())intro_.Commit();initialIndices_=std::move(work.indices);if(work.encoded){activeEncodedPages_=work.keys;activeBooks_=work.books;activePages_.clear();}else{activePages_=work.keys;activeEncodedPages_.clear();activeBooks_.clear();}
